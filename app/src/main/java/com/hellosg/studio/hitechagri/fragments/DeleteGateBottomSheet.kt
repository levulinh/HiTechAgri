...
class DeleteGateBottomSheet : BottomSheetDialogFragment() {

    private var activityCallback: OnDeleteClickListener? = null

    interface OnDeleteClickListener {
        fun onDeleteClick(gate: GateWay)
    }

    companion object {
        fun newInstance(gate: GateWay): DeleteGateBottomSheet {
            val args = Bundle()
            args.putString("name", gate.name)
            args.putString("id", gate.gateId)
            val fragment = DeleteGateBottomSheet()
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.bottom_sheet_delete_gate, container, false)
        ...
        btnDelete.setOnClickListener {
            activityCallback
                ?.onDeleteClick(GateWay(arguments?.getString("name")!!, arguments?.getString("id")!!))
        }
